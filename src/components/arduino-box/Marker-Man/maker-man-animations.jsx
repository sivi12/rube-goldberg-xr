...
export function markerManAnimations(
  actions,
  ...
  const allAnimations = actions["ALLANIMATIONS"];
  const allAnimationsTime = sittingIdle.getClip().duration * 1000 - 500;
  ...
  if (actions && startAnimationButton == "sittingIdle") {
    mixer.stopAllAction();
    sittingIdle.play();
  }

  if (startAnimationButton == "startAnimation") {
    mixer.stopAllAction();
    dance.play();
    // setTimeout(() => {
  ...
  if (startAnimationButton == "endAnimation") {
    if (dance.isRunning) {
      mixer.stopAllAction();
      dance2.play();
    } else {
  ...
  //   if (actions && startAnimationButton) {
